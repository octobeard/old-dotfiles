...
then
    ...
    echo "Copy of audio files successful." >> $LOG_FILE
elif [ $MP4_COUNT -gt 0 ] || [ $MKV_COUNT -gt 0 ]
then
    cp -Rv "$TR_TORRENT_DIR/$TR_TORRENT_NAME" "$MOVIE_DEST" >> $LOG_FILE
    echo "Copy of movie files successful." >> $LOG_FILE
else
    echo "Unknown file types found for torrent. $FLAC_COUNT $MP3_COUNT $MP4_COUNT $MKV_COUNT Skipping copy. $TR_TORRENT_NAME" >> $LOG_FILE
fi
